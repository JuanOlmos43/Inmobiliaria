...
export default function ImageSection({
    ...
    return (
        <div className="space-y-3">
            <label className="block text-sm font-medium text-gray-700">Imágenes ({items.length})</label>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {items.map((item, index) => (
                    <div key={item.id} className="relative group aspect-video bg-gray-100 rounded-lg overflow-hidden border border-gray-200 shadow-sm transition-all hover:shadow-md">
                        {/* Number Badge */}
                        ...
                        <img src={item.url} className="w-full h-full object-cover" alt={`Propiedad ${index + 1}`} />
                ...
                <label className="border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center aspect-video cursor-pointer hover:border-(--accent) hover:bg-gray-50 transition-all group relative overflow-hidden">
                    <div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center group-hover:bg-(--accent)/10 group-hover:text-(--accent) transition-all mb-2 group-hover:scale-110">
                        <span className="text-3xl text-gray-400 group-hover:text-(--accent) leading-none pb-1">+</span>
                    </div>
                    <span className="text-xs text-gray-500 font-medium group-hover:text-(--accent)">Agregar foto</span>
