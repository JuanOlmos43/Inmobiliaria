      ...
      for (const contract of contracts) {
        ...
        for (const recipient of recipients) {
          try {
            ...
            this.logger.log(
              `Expiration alert sent to ${recipient} for contract ${contract.id} (${days} days)`,
            );
          } catch (error) {
            this.logger.error(
              `Failed to send expiration alert to ${recipient} for contract ${contract.id}`,
              error,
            );
          }
        }
    ...
    for (const contract of contracts) {
      ...
      for (const recipient of recipients) {
        try {
          await this.resend.sendEmail(recipient, subject, html);
          this.logger.log(
            `Adjustment alert sent to ${recipient} for contract ${contract.id}`,
          );
        } catch (error) {
          this.logger.error(
            `Failed to send adjustment alert to ${recipient} for contract ${contract.id}`,
            error,
          );
        }
      }
